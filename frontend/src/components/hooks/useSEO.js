...
// 🎯 HOOK ESPECÍFICO PARA TOURS (VERSÃO CORRIGIDA E FINAL)
export const useTourSEO = (tour) => {
  const { getCurrentLanguage } = useTranslation();
  const currentLang = getCurrentLanguage();
  
  // Define valores padrão se o tour for nulo para evitar a chamada condicional de hooks.
  ...
  // O hook `useSEO` é sempre chamado, resolvendo o erro.
  return useSEO({
    title: tourTitle,
